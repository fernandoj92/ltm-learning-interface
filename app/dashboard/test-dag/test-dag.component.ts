...
@Component({
    moduleId: module.id,
    selector: 'test-dag',
    templateUrl: 'test-dag.component.html'
})
export class TestDagComponent implements OnInit {
    ...
    ngAfterViewInit() {
        ...
        var link = d3.select(this.test.nativeElement).selectAll('.link')
            .data(links)
            .enter().append('line')
            .attr('class', 'link');
        
        var node = d3.select(this.test.nativeElement).selectAll('.node')
            .data(nodes)
            .enter().append('circle')
            .attr('class', 'node');
